...
export function genTraversalHandler(fn, setter) {
    if (setter === void 0) { setter = function (target, key, value) { target[key] = value; }; }
...
var myDeepAssign = genTraversalHandler(function (val) { return val; });
...
export function deepAssign() {
    ...
    args.forEach(function (source) { return myDeepAssign(source, target); });
    return target;
}
export function camelize(str, isBig) {
...
export function hypenate(str) {
    return camelize(str).replace(/([A-Z])/g, function (match) { return "-" + match.toLowerCase(); });
}
...
export function getDeepProperty(obj, keys, _a) {
    var _b = _a === void 0 ? {} : _a, _c = _b.throwError, throwError = _c === void 0 ? false : _c, backup = _b.backup;
    if (isString(keys)) {
        keys = keys.split(".");
    }
    if (!isArray(keys)) {
